...
| Part | Qty | Source / link | ADR |
|---|---|---|---|
| Custom universal mounting plate (90.5x80.5x7mm, 4 corner mounting holes + diamond-grid hole/slot pattern) | 6: front+back pair per connection point — 2 pairs (4 plates) for X-axis gantry legs, 1 pair (2 plates) for the Y-axis carriage — see [hardware/laser-cut/20x40.stl](../hardware/laser-cut/20x40.stl) | user-provided design, placed alongside the gantry end caps; sourcing/material (printed vs. machined) not yet decided | [0040](decisions/0040-x-axis-v-wheel-gantry.md), [0039](decisions/0039-y-axis-single-rail-dual-block.md) — **not yet redesigned**: gantry end cap's carriage-attachment zone still cut for MGN12H's bolt pattern, needs redrawing to bolt to this plate's hole pattern instead |
| Delrin Mini V-Wheel kit, assembled (wheel + MR105ZZ bearing + 6mm eccentric spacer) | 2x 10-pack (need 12: 2 X-axis gantry sides x 4 wheels + 4 on the Y-axis carriage, reinforcing the rail against wiggle) | [eBay](https://www.ebay.com/itm/404701554316), $16.00/10-pack x2 = $32.00 (~$1.60/wheel), 8 spares | [0040](decisions/0040-x-axis-v-wheel-gantry.md), [0039](decisions/0039-y-axis-single-rail-dual-block.md) — wheels sandwich the extrusion between a front and back plate, not a single plate resting against one face |
| Standoff hardware (spacers/threaded rod) connecting each front/back plate pair | 3 sets (one per connection point) | generic hardware — length not yet determined | [0040](decisions/0040-x-axis-v-wheel-gantry.md) — **VERIFY**: exact standoff length set once the plate/wheel/extrusion stack is physically test-fitted |
...
- Front/back plate standoff length for the wheel-mount pairs — set once the plate/wheel/extrusion stack is physically test-fitted ([0040](decisions/0040-x-axis-v-wheel-gantry.md), [0039](decisions/0039-y-axis-single-rail-dual-block.md))
