...
| Part | Qty | Source / link | ADR |
|---|---|---|---|
| Custom universal mounting plate (90.5x80.5x7mm, 4 corner mounting holes + diamond-grid hole/slot pattern) | 2 (one per X-axis gantry side) — quantity for the Y-axis carriage still open, see [hardware/laser-cut/20x40.stl](../hardware/laser-cut/20x40.stl) | user-provided design, placed alongside the gantry end caps; sourcing/material (printed vs. machined) not yet decided | [0040](decisions/0040-x-axis-v-wheel-gantry.md) — **not yet redesigned**: gantry end cap's carriage-attachment zone still cut for MGN12H's bolt pattern, needs redrawing to bolt to this plate's hole pattern instead |
| Delrin Mini V-Wheel kit, assembled (wheel + MR105ZZ bearing + 6mm eccentric spacer) | 1x 10-pack (need 8: 2 gantry sides x 4 wheels each) | [eBay](https://www.ebay.com/itm/404701554316), $16.00/10-pack (~$1.60/wheel) | [0040](decisions/0040-x-axis-v-wheel-gantry.md) — wheels bolt to the custom plate above, not a pre-made gantry-plate product |
