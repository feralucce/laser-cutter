...
Date: 2026-07-13
Status: Accepted (mechanical role corrected 2026-07-13, see [0014](0014-axis-naming-convention.md))

## Context

The whole gantry (crossbeam + X-rail + carriage) travels along the fixed
800mm axis on two parallel MGN12 rails ([0008](0008-motion-system-mgn12-rails.md)),
one per side, and needs to stay synchronized so the gantry doesn't rack out
of square. Per [0014](0014-axis-naming-convention.md), this is the **Y**-axis
— matching Ortur's own convention (their extension kit explicitly extends
Y, via GRBL `$131`, to reach 800mm) and the downdraft table's structural
constraint ([0007](0007-frame-footprint-constraints.md)).
...
headroom for that without redesigning the Y-axis drive. This also happens to
be the heaviest-loaded pair of rails in the machine (they carry the whole
gantry, including the X-axis's own rail/carriage/motor) — putting dual
motors and the shorter 800mm span here, rather than on the longer X-axis
span, minimizes deflection risk for the heaviest load in the system.
