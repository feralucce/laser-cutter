...
**Confirmed Ortur convention** (via Ortur's own product listings for the
Y-axis extension kit used to reach 800mm — see prior conversation
research): **Y = the long axis**, the one with the guide rails the whole
gantry beam travels along (what this project's [0014](0014-axis-naming-convention.md)
called "X"). **X = the short axis**, the beam-mounted carriage axis
(what [0014](0014-axis-naming-convention.md) called "Y").

## Decision

Adopt Ortur's own convention going forward:

- **Y-axis = the long axis (1200mm).** Two guide rails, dual NEMA17
  motors mechanically coupled by a connecting drive shaft
  ([0041](0041-ortur-electronics-reuse.md)), whole gantry (beam +
  carriage) travels along these.
- **X-axis = the short axis (800mm).** Single MGN12 rail on the gantry
  beam itself, single NEMA17 motor, laser carriage (holding the K40)
  travels along this.
...
convention (Y = long/dual-motor, X = short/carriage) directly, no swap
needed.
...
- Wiring: when the reused Ortur harness is physically connected, its
  ...
  convention (Y = long axis) to whatever the harness's own connector
